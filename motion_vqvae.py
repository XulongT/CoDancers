...
import models
import datetime
# from utils.metrics_new import fid
import json
    ...
    def eval(self):
        with torch.no_grad():
            ...
            for i_eval, batch_eval in enumerate(tqdm(self.test_loader, desc='Generating Dance Poses')):
                ...
                src_pos_eval = pose_seq_eval.reshape(-1, pose_seq_eval.size(2), pose_seq_eval.size(3))
                ...
                bk, t, c = src_pos_eval.size()
                # t = t - 1

                # diffgt = (src_pos_eval[:, 1:] - src_pos_eval[:, :-1]).view(bk, t-1, c//3, 3)

                pose_seq_out, loss, _ = model(src_pos_eval)
                print(f"pso{pose_seq_out.shape}")
                # diffout = (pose_seq_out[:, 1:] - pose_seq_out[:, :-1]).view(bk, t-1, c//3, 3)

                # diffgt2 = (src_pos_eval[:, 2:] + src_pos_eval[:, :-2] - 2 * src_pos_eval[:, 1:-1]).view(bk, t-2, c//3, 3)
                # diffout2 = (pose_seq_out[:, 2:] + pose_seq_out[:, :-2] - 2 * pose_seq_out[:, 1:-1]).view(bk, t-2, c//3, 3)

                diff = (src_pos_eval - pose_seq_out).view(bk, t, c // 3, 3)
                ...
                if config.structure.use_bottleneck:
                    ...
                    if isinstance(quants_pred, tuple):
                        quants[self.test_dance_names[i_eval]] = (
                        model.module.encode(src_pos_eval)[0][0].cpu().data.numpy()[0],
                        model.module.encode(src_pos_eval)[1][0].cpu().data.numpy()[0])
                    else:
                        quants[self.test_dance_names[i_eval]] = model.module.encode(src_pos_eval)[0].cpu().data.numpy()[0]
                else:
                    quants = None
                ...
                # plt.close()
                # exit()
            visualizeAndWrite(results, config, self.evaldir, self.test_dance_names, epoch_tested, quants,
                                  self.test_person_dict)
