...
def visualize(config, the_dance_names, expdir, epoch, quants=None, worker_num=16):
    ...
    # print("Visualizing")
    for index, i in enumerate(tqdm(range(0, len(dance_names), 7), desc='Generating Images')):

        dance_name = dance_names[i]
        print(json_dir, dance_name)
        ...
        People_List = []
        for i in range(7):
            json_dr = dance_path.replace('dancer0', 'dancer{}'.format(str(i)))
            json_files = sorted(os.listdir(json_dr))
            flag = False
            for j in range(0, len(json_files), int(len(json_files) / 6)):
                json_file_path = os.path.join(json_dr, json_files[j])
                with io.open(json_file_path, encoding='utf-8') as f:
                    root = json.loads(f.read())["people"][0]["pose_keypoints_2d"][:2]
                    if not (478.5 < root[0] < 479.5 and 365 < root[1] < 366):
                        flag = True
                        break
            if flag == True:
...
def write2pkl(dance, dance_name, config, expdir, epoch, person_index, rotmat):
    epoch = int(epoch)
    import torch
    ...
    if not os.path.exists(ep_path):
        os.makedirs(ep_path)
    pkl_data = {"pred_position": dance}
    dance_path = os.path.join(ep_path, f"{dance_name}_dancer{person_index}.pkl")
...
def pose_code2pkl(pcodes, dance_names, config, expdir, epoch):
    epoch = int(epoch)
    print(f"dance names length: {len(dance_names)}")
...
def write2json(dances, dance_names, config, expdir, epoch):
    ...
    for i in tqdm(range(len(dances)), desc='Generating Jsons'):
        ...
        print(f"dance nm{dance_names[i]}")
        if not os.path.exists(dance_path):
            os.makedirs(dance_path)
...
def visualizeAndWrite(results, config, expdir, dance_names, epoch, quants=None, person_dict=None):
    if config.rotmat:
    ...
    if config.data.name == "aist":
        ...
        for i in range(len(results)):
            for person_j in range(len(results[0])):
                dance_name = dance_names[i]
                real_people = person_dict[dance_name]
                np_dance = results[i][person_j].data.cpu().numpy()
                if config.rotmat:
                    print('Use SMPL!')
                    root = np_dance[:, :3]
                    ...
                    write2pkl(np_dance, dance_name, config.testing, expdir, epoch, person_j, rotmat=config.rotmat)
...
def load_data_aist(data_dir, interval=120, move=40, rotmat=False, external_wav=None, external_wav1=None, external_wav_rate=1, music_normalize=False, wav_padding=0):
    # tot = 0
    all_data = []
    fnames = sorted(os.listdir(data_dir))
    # print(fnames)
    ...
    def process(fname):
        ...
        try:
            with open(path, 'r') as f:
                sample_dict = json.loads(f.read())
                np_music = np.array(sample_dict.get('music_array', [])) 
                ...
                if external_wav1 is not None:
                    wav_path1 = os.path.join(external_wav1, fname)
                    # print('load from external wav!')
                    with open(wav_path1) as ff1:
                        sample_dict_wav1 = json.loads(ff1.read())
                        np_music1 = np.array(sample_dict_wav1['music_array']).astype(np.float32)

                np_dance = np.array(sample_dict['dance_array'])
                person_k = sample_dict['persons']

                if not rotmat:
                    root = np_dance[:, :, :3]  # the root
                    np_dance = np_dance - np.tile(root, (1, 1, 24))  # Calculate relative offset with respect to root
                    # print(f"np- shape{np_dance.shape}")
                    np_dance[:, :, :3] = root

                music_sample_rate = external_wav_rate if external_wav is not None else 1
                import math
                if interval is not None:
                    # seq_len, dim = np_music1.shape
                    seq_len = min(np_music.shape[0], np_music1.shape[0])
                    np_music = np.concatenate((np_music[:seq_len, :], np_music1[:seq_len, :]), axis=1)
                    seq_len, dim = np_music.shape
                    for i in range(0, seq_len, move):
                        ...
                        # music_sub_seq = np.concatenate((music_sub_seq, music_sub_seq1), axis=1)
                        ...
                        if music_sub_seq.shape[0] == interval_sample and dance_sub_seq.shape[1] == interval:
                            ...
                            with lock:
                                all_data.append({'music': music_sub_seq_pad, 'dance': dance_sub_seq, 'person_num': person_k})
                else:
                    music_data.append(np_music)
                    dance_data.append(np_dance)
    ...
    with ThreadPoolExecutor(max_workers=16) as executor:
        executor.map(process, fnames)
    music_data, dance_data, sub_person_list = [], [], []
    input_names = fnames
    for i in range(len(all_data)):
        music_data.append(all_data[i]['music'])
        dance_data.append(all_data[i]['dance'])
        sub_person_list.append(all_data[i]['person_num'])
    ...
    return music_data_norm, dance_data, ['11', '22', ], input_names, sub_person_list

def load_test_data(data_dir, data_type='2D'):
    tot = 0
    music_data, dance_data, person_list = [], [], []
    fnames = sorted(os.listdir(data_dir))
    for fname in fnames:
        path = os.path.join(data_dir, fname)
        with open(path) as f:
...
def load_test_data_aist(data_dir, rotmat, move, external_wav=None, external_wav1=None, external_wav_rate=1, music_normalize=False, wav_padding=0):
    tot = 0
    input_names = []
    ...
    for fname in fnames:
        ...
        with open(path) as f:
            ...
            if external_wav1 is not None:
                ...
                with open(wav_path1) as ff1:
                    sample_dict_wav1 = json.loads(ff1.read())
                    np_music1 = np.array(sample_dict_wav1['music_array'])

            if 'dance_array' in sample_dict:
                np_dance = np.array(sample_dict['dance_array'])
                person_k = sample_dict['persons']
                person_list.append(person_k)
                if not rotmat:
                    root = np_dance[:, :, :3]  # the root
                    np_dance = np_dance - np.tile(root, (1, 1, 24))  # Calculate relative offset with respect to root
                    np_dance[:, :, :3] = root
                    # print(f"npd1 shape: {np_dance.shape}")

                for kk in range((np_dance.shape[1] // move + 1) * move - np_dance.shape[1]):
                    np_dance = np.append(np_dance, np_dance[:, -1:, :], axis=1)

                # print(f"npd shape: {np_dance.shape}")
                dance_data.append(np_dance)
            ...
            # zero padding left
            for kk in range(wav_padding):
                np_music1 = np.append(np.zeros_like(np_music1[-1:]), np_music1, axis=0)
            # fully devisable
            for kk in range((len(np_music1) // music_move + 1) * music_move - len(np_music1)):
                np_music1 = np.append(np_music1, np_music1[-1:], axis=0)
            # zero padding right
            for kk in range(wav_padding):
                np_music1 = np.append(np_music1, np.zeros_like(np_music1[-1:]), axis=0)

            import math
            music_len = min(np_music.shape[0], np_music1.shape[0])
            np_music = np.concatenate((np_music[:music_len, :], np_music1[:music_len, :]), axis=1)
    ...
    return music_data_norm, dance_data, input_names, person_list
...
def visualizeAndWritefromPKL(pkl_root, config=None):
    ...
    np_dances_original = []
    dance_datas = []
    for pkl_name in sorted(os.listdir(pkl_root)):
